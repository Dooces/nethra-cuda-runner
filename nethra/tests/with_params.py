"""usage: [LEAK=2] [COND=top|all|top_and_leaves] python3 with_params.py script.py args...  (runs a
nethra/tests script with a different default leakage / conduction for every NethraField it creates)"""
import os, sys, runpy
...
LEAK=float(os.environ.get("LEAK","1")); COND=os.environ.get("COND")
