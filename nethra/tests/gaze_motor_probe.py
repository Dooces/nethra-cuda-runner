"""Expose closed-loop with the retina reflex (as gaze_retina_device.py).  Then, on frozen copies at each of the
last 2*L exposure intervals, read motor Nethra activation mL, mR after the interval, exposed vs lesioned
...
for t in range(EXPOSE):
    ...
    if t>=EXPOSE-2*L: rows.append((t,pre,w.m,vals))
print(f"DIR={DIR}: t (x,e,m now) -> next m by reflex | exposed mL mR | lesioned mL mR | structure adds mL mR")
