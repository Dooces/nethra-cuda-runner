...
Native confidence rule:
  1. grounded AAPL prospective current and coupling-weighted higher-AAPL Nethra resonance agree
     in sign;
  2. |coupling-weighted higher-field resonance| >= fixed cutoff learned from calibration magnitudes.
...
def quantile_cutoffs(rows):
    vals=[]
    for r in rows:
        g=float(r["ground"]); c=float(r["coupled"])
...
def main():
    ...
    result={
        ...
        "calibration_all_coupled":summarize(cal,None,False),
        "evaluation_all_coupled":summarize(test,None,False),
